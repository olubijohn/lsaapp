...
interface Modern01TemplateProps {
...
export function Modern01Template({
  settings,
  billFrom,
  billTo,
  document,
  ...
  return (
    <div className="invoice-document relative bg-white shadow-lg  rounded-lg overflow-visible">
      <div
      ...
      >
        ...
        <div
          className="pt-10 p-4 rounded-t-lg"
          style={{ backgroundColor: settings.themeColor }}
        >
          <div className="grid grid-cols-2 gap-x-4 gap-y-3">
            <div className="flex flex-col gap-1">
              <p className="text-white/80 font-semibold text-xs">Bill From:</p>
              <p className="text-white font-bold text-left">{billFrom.name}</p>
            </div>
            <div className="flex flex-col gap-1">
              <p className="text-white/80 font-semibold text-xs text-right">
                Bill To:
              </p>
              <p className="text-white font-bold text-right">{billTo.name}</p>
            </div>
            <div className="flex flex-col gap-1">
              <p className="text-white/90 font-medium text-xs text-left">
                Email: {billFrom.email || "N/A"}
              </p>
            </div>
            <div className="flex flex-col gap-1">
              <p className="text-white/90 font-medium text-xs text-right">
            ...
            <div className="flex flex-col gap-1">
              <p className="text-white font-medium text-xs text-right">
            ...
            <div className="flex flex-col gap-1">
              <p className="text-sm font-medium text-white/90 text-right">
        ...
        <div className="px-6 pt-6 pb-6 flex flex-col gap-6">
          ...
          <div className="rounded-md border border-dashed border-[#B9B9C5]">
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                >
                  <tr>
                    <th className="px-4 py-2 text-xs font-semibold text-left w-[5%]">
                      No.
                    </th>
                    <th className="px-4 py-2 text-xs font-semibold text-left w-[35%]">
                      Item
                    </th>
                    <th className="px-4 py-2 text-xs font-semibold text-center w-[12%]">
                      Quantity
                    </th>
                    <th className="px-4 py-2 text-xs font-semibold text-right w-[15%]">
                      Unit Price
                    </th>
                    <th className="px-4 py-2 text-xs font-semibold text-right w-[10%]">
                      Discount
                    </th>
                    <th className="px-4 py-2 text-xs shrink-0 text-nowrap font-semibold text-center w-[10%]">
                      Tax code
                    </th>
                    <th className="px-4 py-2 text-xs font-semibold text-right w-[13%]">
                      Total
                    </th>
                  </tr>
                </thead>
              ...
              <div>
                <table className="w-full">
                  <thead
                    ...
                    style={{ backgroundColor: settings.themeColor }}
                  >
                    <tr className="h-fit font-semibold">
                      <th className="px-2 py-[5px] text-xs text-left whitespace-nowrap">
                        Tax code
                      </th>
                      <th className="px-2 py-[5px] text-xs text-left">Tax</th>
                      <th className="px-2 py-[5px] text-xs text-left whitespace-nowrap">
                        Net Amount
                      </th>
                    </tr>
                  ...
                  <tbody className="text-xs font-medium">
                    {taxItems.map((item) => (
                      <tr
                        key={item.taxCode}
                        className="border-b border-dashed border-gray-50 text-sm text-right"
                      >
          ...
          <div className="flex justify-between items-start gap-8 text-text-tertiary font-medium text-xs mb-2">
            <div className="flex-1 ">
              <h6 className="font-medium text-text-tertiary text-xs mb-2">
                Payment Details
              </h6>
          ...
          <div className="mt-4">
            ...
            >
              ...
              <h1 className="text-3xl font-bold text-black">{documentTitle}</h1>
            </div>
          </div>
